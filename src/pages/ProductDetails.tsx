...
const ProductDetails: React.FC = () => {
        ...
        const fetchProduct = async () => {
            try {
                const response = await axios.get(
                    "https://round-3-pet-store.digital-vision-solutions.com/api/pets"
                );
                console.log("Products response:", response.data);
                const firstProduct = response.data.data[0]; 
                setProduct({
                    images: firstProduct?.images || [],
                    name: firstProduct?.name || "undefined",
                    price: firstProduct?.price || "undefined",
                    gender: firstProduct?.gender,
                    age: firstProduct?.age,
                    size: firstProduct?.size,
                    color: firstProduct?.color,
                    vaccinated: firstProduct?.vaccinated,
                    dewormed: firstProduct?.dewormed,
                    cert: firstProduct?.cert,
                    microchip: firstProduct?.microchip,
                    location: firstProduct?.location,
                    publishedDate: firstProduct?.publishedDate,
                    additionalInfo: firstProduct?.additionalInfo,
                });
            } catch (error) {
    ...
    useEffect(() => {
        setCustomers([{ image: "https://round-3-pet-store.digital-vision-solutions.com/storage/customers/customer1.jpg" }]);
    }, []);
                    ...
                    {product ? (
                        <>
                            ...
                            <div className="flex flex-row sm:flex-row gap-4 w-full mt-6 ">
                                <button className="bg-transparent w-full border border-gray-900 text-sm sm:text-base text-gray-900 font-semibold transition-all duration-400 p-2 rounded-full hover:bg-gray-900 hover:text-white justify-center flex items-center cursor-pointer">
                                    Contact us 
                                </button>

                                <button className="bg-gray-900 p-1 w-full text-white text-sm sm:text-base rounded-full hover:bg-transparent hover:border hover:border-gray-900 hover:text-black transition-all duration-400 cursor-pointer">
                            ...
                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                ...
                                <p>{product.gender || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">Age:</p>
                                <p>{product.age || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">Size:</p>
                                <p>{product.size || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">Color:</p>
                                <p>{product.color || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">Vaccinated:</p>
                                <p>{product.vaccinated || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">Dewormed:</p>
                                <p>{product.dewormed || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">Cert:</p>
                                <p>{product.cert || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">Microchip:</p>
                                <p>{product.microchip || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">Location:</p>
                                <p>{product.location || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">PublishedDate:</p>
                                <p>{product.publishedDate || "undefined"}</p>
                            </div>

                            <div className="flex text-[#667479] justify-between border-b border-b-gray-200 mt-5">
                                <p className="font-bold">Additional Information :</p>
                                <p>{product.additionalInfo || "undefined"}</p>
                            </div>
                        </>
                    ) : (
